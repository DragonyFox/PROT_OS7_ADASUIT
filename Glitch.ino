    ...
    if (millis() - glitchDelay > 7000) {
      if (millis() - glitchSpeed > 100) {
        ...
        if (counter == 1) {
          ...
          matrix.drawBitmap(MawLeft, 0, Glitch1L, 32, 8, PCOLOR);
          matrix.drawBitmap(MawRight, 0, Glitch1, 32, 8, PCOLOR);
          matrix.show();
        }
        if (counter == 2) {
          matrix.fillRect(MawLeft, 0, 32, 8, 0);
          matrix.fillRect(MawRight, 0, 32, 8, 0);
          matrix.drawBitmap(MawLeft, 0, Glitch2L, 32, 8, PCOLOR);
          matrix.drawBitmap(MawRight, 0, Glitch2, 32, 8, PCOLOR);
          matrix.show();
        }
        glitchSpeed = millis();
      }
